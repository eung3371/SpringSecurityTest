...
/* 설명. 결국 Spring Security 관련 설정도 IoC 컨테이나가 인식해야 하는 설정 정보이므로 해당 어노테이션이 필요하다. */
@Configuration
...
public class SecurityConfig {
    ...
    public SecurityFilterChain filterChainConfiguration(HttpSecurity http) throws Exception {
        ...
        // #2 로그인 관리 : <form>태그를 사용한 로그인(form-login) 관련 설정
            ...
            // 로그인 성공시 이동할 기본 페이지(로그인 성공 페이지에 해당되는 핸들러 매핑이 존재해야 함)
            login.defaultSuccessUrl("/", true);
            // 로그인 실패 시, 해당 예외를 처리할 핸들러를 지정(직접 제작한 핸들러 사용)
            login.failureHandler(authFailHandler);
        // #3. 로그아웃 관리 : 로그아웃 요청 시 관련 처리 설정
        }).logout(logout -> {
            // 로그아웃 경로 설정
            logout.logoutRequestMatcher(new AntPathRequestMatcher("/auth/logout"));
            // 로그아웃 시, 클라이언트 JESSIONID 쿠키 삭제
